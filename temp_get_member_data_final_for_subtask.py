def get_member_data(member_id):
    ...
    try:
        ...
        # 1. Initialize response_accounts and response_transactions
        response_accounts = []
        response_transactions = {}
        
        # 2. Process DNA accounts
            ...
            for acc_detail_dict in dna_data.get('accounts', []): 
                try:
                    ...
                    if acc_num and acc_num in member_transactions_in_cache:
                        ...
                        # Ensure transactions are strings, formatting as needed.
                        formatted_txns = [str(tx) if not isinstance(tx, str) else tx for tx in raw_txns]
        ...
        # 3. Process MeridianLink (ML) loan accounts
        if ml_data and isinstance(ml_data, list):
            for loan_detail_dict in ml_data:
                try:
                    ...
                    # Initialize with an empty list if not already present from DNA account processing.
        ...
        # 4. Fallback/Consistency Check: Ensure all accounts listed in response_accounts have a corresponding transaction list
        for key_str in response_accounts:
            if key_str not in response_transactions:
                response_transactions[key_str] = []
                
        # 5. Get cached AI insights
        insights_list = []
        ...
        # 6. Construct member_info, preferring DNA data but falling back to DB record
        ...
        logging.info(f"[API] Successfully prepared data for member_id: {member_id} (MemberNumber: {member_number}). Accounts: {len(response_accounts)}, Transactions keyed by: {list(response_transactions.keys())}")

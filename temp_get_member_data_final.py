...
def get_member_data(member_id):
    ...
    try:
        ...
        # This is important if, for instance, only ML data was available, or if transaction_cache was empty.
        ...
        # Construct member_info, ensuring dna_data is checked for None before access
        member_name = record.get('Name', 'Unknown') # Default to DB name
        if dna_data:
        ...
        if dna_data: # Add other dna_data fields if they exist
            member_info_data.update({k: v for k, v in dna_data.items() if k not in ['accounts', 'firstname', 'lastname']})

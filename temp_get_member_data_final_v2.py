...
def get_member_data(member_id):
    ...
    try:
        ...
        if dna_data and isinstance(dna_data.get('accounts'), list):
            for acc_detail in dna_data['accounts']:
                try:
                    acc_num = acc_detail.get('account_number')
                    acc_type = acc_detail.get('account_type', 'Account')
                    acc_balance = acc_detail.get('balance', '0.00')
                    
                    account_key_string = f"{acc_type} - ${acc_balance}"
                    response_accounts.append(account_key_string)
                    
                    # Retrieve transactions for this DNA account from cache
                    member_transactions_in_cache = transaction_cache.get(member_number, {})
                    if acc_num in member_transactions_in_cache:
                        response_transactions[account_key_string] = member_transactions_in_cache[acc_num][:8]
                    else:
                        response_transactions[account_key_string] = []
                except Exception as e:
                    logging.error(f"[API] Error processing DNA account detail {acc_detail} for member {member_number}: {e}", exc_info=True)
        
        # Process MeridianLink (ML) loan accounts
        if ml_data and isinstance(ml_data, list):
            for loan_detail in ml_data:
                try:
                    loan_type = loan_detail.get('loan_type', 'Loan')
                    loan_num = loan_detail.get('loan_num', '') # Could be an application ID or loan number
                    
                    account_key_string = f"{loan_type} #{loan_num}"
                    response_accounts.append(account_key_string)
                    
                    # Loans from ML generally don't have transactions in the DNA transaction_cache.
                    # Initialize with an empty list if not already present (e.g., if somehow it's also a DNA account).
                    if account_key_string not in response_transactions:
                        response_transactions[account_key_string] = []
                except Exception as e:
                    logging.error(f"[API] Error processing ML loan detail {loan_detail} for member {member_number}: {e}", exc_info=True)

        # Ensure all accounts listed in response_accounts have a corresponding transaction list (even if empty)
        ...
        if member_id in insight_cache: # member_id is the checkin_id
            ...
            elif isinstance(insights_text, list): # If already a list
        ...
        member_name = record.get('Name', 'Unknown Member') # Default to DB name from check-in
        if dna_data:
            firstname = dna_data.get('firstname', '')
            lastname = dna_data.get('lastname', '')
            if firstname or lastname: # Only override if DNA has a name
        ...
        # Add other relevant details from dna_data to member_info if available
        if dna_data:
        ...
        return jsonify({
            ...
            'insights': insights_list[:4],  # Limit to 4 insights for the UI

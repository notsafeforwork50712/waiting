...
def get_member_data(member_id):
    ...
    try:
        ...
        if dna_data and isinstance(dna_data.get('accounts'), list):
            for acc_detail_dict in dna_data.get('accounts', []): # Use .get with default for safety
                try:
        ...
        if ml_data and isinstance(ml_data, list):
            for loan_detail_dict in ml_data:
                try:
                    ...
                    loan_num = loan_detail_dict.get('loan_num', '') # Could be an application ID or loan number
